...
<div class="navbar navbar-inverse navbar-fixed-top">
  <div class="navbar-inner">
    <div class="container">
      <a class="brand" href="#">Sense for Glass (adapted from Glassware Start Project)</a>
    </div>
  </div>
</div>
    ...
    <div class="span4">
      ...
      <p>When you first sign in, this Glassware inserts a welcome message. Use
        ...
        <form action="<%= WebUtil.buildUrl(request, "/main") %>" method="post">
            <input type="hidden" name="operation" value="queryRemoteSensor">
            <button class="btn btn-block" type="submit">
                Start Query Sensor</button>
        </form>
...
<%--
      <form action="<%= WebUtil.buildUrl(request, "/main") %>" method="post">
        ...
        <input type="hidden" name="imageUrl" value="<%= appBaseUrl +
               "static/images/chavez001_Rose.jpg" %>">
    ...
    <div class="span4">
      ...
      <form action="<%= WebUtil.buildUrl(request, "/main") %>" method="post">
        ...
        <button class="btn btn-block btn-success" type="submit">
          Insert <%= MainServlet.CONTACT_NAME %> Contact
        </button>
      </form>
      <% } else { %>
      <form action="<%= WebUtil.buildUrl(request, "/main") %>" method="post">
        ...
        <button class="btn btn-block btn-danger" type="submit">
          Delete <%= MainServlet.CONTACT_NAME %> Contact
        </button>
      </form>
      <% } %>

      <h3>Voice Commands</h3>
      <p>The "<%= MainServlet.CONTACT_NAME %>" contact also accepts the <strong>take a
        note</strong> command. Take a note with the "<%= MainServlet.CONTACT_NAME %>" contact
